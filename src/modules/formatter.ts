...
function format(commands, wrap) {
    ...
    if (wrap) {
        commandsAsText = wrapInDefineFeature(commandsAsText).join(newLine);
    }
    return formatJavascript(commandsAsText);
}

function wrapInDefineFeature(commandsAsText) {
    const commandsInDefineFeature = ["defineFeature(feature, test => {", commandsAsText, "});"];
    return commandsInDefineFeature;
}

function formatJavascript(commands) {
    const formatedJavascript = js(commands, {
        indent_size: 4,
